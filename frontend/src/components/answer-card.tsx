        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-8 gap-4">
            ...
            <div className="flex items-center gap-2">
              {(isEnhanced
                ? ["Entity Extraction", "SKOS Expansion", "Vector Search", "Structured Lookup", "LLM"]
                : ["Vector Search", "LLM"]
              ).map((step, i) => (
                <span key={i} className="contents">
                  {i > 0 && <span className="text-xs text-muted-foreground">→</span>}
                  <span
                    className={`inline-block rounded-full px-2.5 py-1 text-[0.65rem] font-medium border ${
                      isEnhanced
            ...
            <div className={`h-6 w-6 rounded-full border-2 border-t-transparent animate-spin ${
              isEnhanced ? "border-amber-500" : "border-gray-400"
            }`} />
            <p className="text-xs text-muted-foreground">
              {isEnhanced ? "Running ontology-enhanced pipeline…" : "Running basic RAG pipeline…"}
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Card className={`${isEnhanced ? "border-l-4 border-l-amber-500" : "border-l-4 border-l-gray-300"}`}>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Search Query Used */}
        {result.search_query && (
          <CollapsibleSection title="Actual Search Query Sent to Vector DB">
        ...
        {result.structured_data.length > 0 && (() => {
          ...
          const sourceValue = dataSourceItem?.value || "";
          const isLiveGSA = sourceValue.includes("GSA") && sourceValue.includes("live");
          const isUSAspending = sourceValue.includes("USAspending") && sourceValue.includes("live");
          const title = isUSAspending
          ...
          const footnote = isUSAspending
            ? "These contracts are queried live from USAspending.gov — not generated by the LLM."
            : isLiveGSA
          ...
          return (
            <CollapsibleSection title={title}>
              {result.structured_query && (
                <div className="mb-3 rounded-md bg-slate-900 px-3 py-2">
                  <div className="text-[0.6rem] font-semibold text-slate-400 uppercase tracking-wider mb-1">
                    Ontology → Structured Query
                  </div>
                  <code className="text-[0.7rem] font-mono text-emerald-400 break-all leading-relaxed">
                    {result.structured_query}
                  </code>
                </div>
              )}
